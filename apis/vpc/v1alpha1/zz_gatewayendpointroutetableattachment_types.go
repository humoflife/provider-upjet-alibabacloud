...
type GatewayEndpointRouteTableAttachment struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.gatewayEndpointId) || (has(self.initProvider) && has(self.initProvider.gatewayEndpointId))",message="spec.forProvider.gatewayEndpointId is a required parameter"
